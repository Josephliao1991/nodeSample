...
function readCSVFile(fileName) {
  // body...
  return fs.createReadStream(fileName+'.csv');
...
function saveToCSV(fileName, acce, gyro) {
  ...
  json2csv({data:acce_json, feilds: fields},function (error, csv) {
    ...
    // csv.writeFile
    fs.writeFile(__dirname +'/csv/'+fileName+'_accs'+'.csv',csv,function (error) {
      // body...
      if (error) {
        console.log(error);
      }
  ...
  json2csv({data:gyro_json, feilds: fields},function (error, csv) {
    ...
    fs.writeFile(__dirname +'/csv/'+fileName+'_gyro'+'.csv',csv,function (error) {
      // body...
      if (error) {
        console.log(error);
      }
